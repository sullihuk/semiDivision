...
int main()
{
  ...
  start = clock();
	...
	printf("x = %f\n", dihot(1, 8, 0.0001, 0.0001));// Вывод на экран решения уравнения. Параметром вывода является результат метода реализующего, собственно, метод дихотомии. В свою очередь, параметрами функции являются начало и конец отрезка, на котором находится решение(Вариант 9), точность с которой будет решаться уравнение, значение, с которым будет сравниваться значение заданной функции на предмет приближения к нулю (по известной теореме, это и будет решением уравнения) соответственно.
  end = clock();
  double time_taken = ((double)(end - start))/CLOCKS_PER_SEC*1000;
  //printf("The value of function at the point of x: %.f\n",func(dihot(2, 8, 0.0001, 0.0001)));
  printf("The number of iteration is: %0.f\n",n);
  printf("The taken time is: %0.f seconds\n",time_taken);
}
